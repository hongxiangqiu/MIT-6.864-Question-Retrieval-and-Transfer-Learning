...
from src.lstm import CombinedEncoder
...
session_id = "{}{:02d}{:02d}_{:02d}{:02d}_".format(now.year, now.month, now.day, now.hour, now.minute) + str(
    int(time.time()))
result_folder = "model_output/dan_bilstm/" + \
                session_id + \
...
model = SimpleDA(CombinedEncoder(encoding_size, glove840b.emb_size, bidirectional=True),
